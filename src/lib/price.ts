...
/**
 * The price after a vendor's sitewide coupon is applied. Used only when the
 * offer itself carries no observed `salePrice` (see `hasDiscount`), a coupon
 * is a rule ("10% off with code X"), not a scraped fact, so it never overrides
 * a real recorded price, only fills in for a plain list price.
 */
export function couponAdjustedPrice(price: Cents, percentOff: number): Cents {
  return Math.round(price * (1 - percentOff / 100)) as Cents;
}

/** Cost per mg for a price other than the listing's own effective price, e.g. after a coupon. */
export function pricePerMgFor(price: Cents, listing: Pick<Listing, 'vialSize' | 'vialCount'>): number | null {
  const totalMcg = listing.vialSize * listing.vialCount;
  if (totalMcg <= 0 || price <= 0) return null;
  return price / (totalMcg / 1000);
}
